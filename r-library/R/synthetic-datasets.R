...
addBootstrappedObservations = function(growthSimulation, bootstraps = 100, delayMean = 0, delaySd = 1, lastObservation = nrow(growthSimulation$ts)-1) {
  ...
  output = tibble(bootstrap = 1:bootstraps) %>% inner_join(datedProb, by=character())
...
getTwoAlternativesDataset = function(
  ...
  combinedTs = scenario1Ts$ts %>% inner_join(scenario2Ts$ts, by=c("time","date","observationDate","bootstrap"), suffix=c(".s1",".s2")) 
